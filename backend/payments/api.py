...
class PaymentsController:
    ...
    @route.get("/subscription", response=UserSubscriptionSchema, auth=JWTAuth(), operation_id="get_user_subscription")
    ...
    @route.post("/checkout", response=CheckoutSessionResponseSchema, auth=JWTAuth(), operation_id="create_checkout_session")
    def create_checkout_session(self, request, data: CreateCheckoutSessionSchema):
        ...
        try:
            ...
            raise APIException(detail="Plan not found", code=404)

        # Check if user already has active subscription
        existing_subscription = StripeService.get_user_subscription(request.user)
        if existing_subscription:
            raise APIException(detail="User already has an active subscription", code=400)
        ...
        try:
            ...
            raise APIException(detail=f"Error creating checkout session: {str(e)}", code=500)

    @route.post("/cancel", response=CancelSubscriptionResponseSchema, auth=JWTAuth(), operation_id="cancel_subscription")
    ...
    @route.post("/portal", response=CustomerPortalResponseSchema, auth=JWTAuth(), operation_id="create_customer_portal")
    def create_customer_portal(self, request, data: CustomerPortalSchema):
        ...
        try:
            ...
            raise APIException(detail=f"Error creating customer portal: {str(e)}", code=500)
